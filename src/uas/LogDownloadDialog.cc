...
//                                  ,tr("Error saving %1.\n file save is corupt."),QMessageBox::Ok);
        ...
        m_downloadSet->insert(static_cast<uint>(floor((double)ofs / LOG_PACKET_SIZE)));
    ...
    if ( count==0 || (count<LOG_PACKET_SIZE
                      && (static_cast<uint>(m_downloadSet->count()) == 1+(static_cast<uint>( floor((double)ofs / LOG_PACKET_SIZE) ))))){
